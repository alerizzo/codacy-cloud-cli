...
| 2026-06-18 | `repo --output json` now includes `repository.fileCount`, derived from `listCommitFiles(limit=1)` on the last analysed commit; best-effort (omitted when no analysed commit or the call fails). Unlocks repo-size visibility for downstream consumers like the `configure-codacy-cloud` skill (2 new tests, 374 total) |
